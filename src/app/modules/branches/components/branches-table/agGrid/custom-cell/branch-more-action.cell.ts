...
@Component({
  ...
  template: `
    ...
    <mat-menu #menu="matMenu" class="more-actions-menu">
      ...
      <button mat-menu-item (click)="updateBranch()" authorization [roleValue]="role" [authName]="authCanUpdate">
        <mat-icon color="primary" class="menu-item" svgIcon="pen"></mat-icon>
        <span>{{ 'table.branch.update_data_branch' | translate }}</span>
      </button>
      <button mat-menu-item (click)="deleteBranch()" authorization [roleValue]="role" [authName]="authCanDelete">
...
export class BranchesMoreActionsCell extends CoreBaseComponent implements ICellRendererAngularComp {
  params!: ICellRendererParams;
  role = IRoleEnum.Branch;
  constructor(
    @Inject(INJECTOR) injector: Injector,
    private _router: Router, private _branchService: BranchService) {
    super(injector);
  }
  ...
  displayDetails() {
    this._branchService.sideBranchDetails.next(this.params.data);
  }
  updateBranch() {
    this._router.navigateByUrl(`${AppRoutes.layout}/${AppRoutes.branches}/update-branch/${this.params.data.id}`);
  }

  deleteBranch() {
    this._branchService
      .deleteBranch(this.params.data.id)
