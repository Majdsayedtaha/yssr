...
@Component({
  selector: 'elect-auth-more-actions',
  template: `
...
export class ElectronicAuthMoreActions extends CoreBaseComponent implements ICellRendererAngularComp {
  params!: ICellRendererParams;
  role = IRoleEnum.DelegationRequest;

  constructor(
    @Inject(INJECTOR) injector: Injector,
    private _electAuthnService: ElectronicAuthService,
  ...
  viewWaiverRequestDetails() {
    this._electAuthnService.requestId.next(this.params.data.id);
  }
  openUpdateContract() {
    this._router.navigateByUrl(`${AppRoutes.layout}/delegations/update/${this.params.data.id}`);
  }
  ...
  deleteRequestConfirmation() {
    ...
    this._toast
      ...
      .pipe(
        switchMap(res => {
          if (res) {
            return this._electAuthnService.deleteWaiverRequest(this.params.data.id);
          } else {
            return of(null);
          }
        })
