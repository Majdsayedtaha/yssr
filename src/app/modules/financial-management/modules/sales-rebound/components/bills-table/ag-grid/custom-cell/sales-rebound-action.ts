...
@UntilDestroy()
@Component({
  selector: 'app-bills-rebound-more-actions',
  template: `
...
export class BillsReboundMoreActionsCell extends CoreBaseComponent implements ICellRendererAngularComp {
  ...
  updateBill() {
    this._router.navigateByUrl(
      `${AppRoutes.layout}/${AppRoutes.financial}/sales-rebound/update-bill/${this.params.data.id}`
    );
  }

  deleteBill() {
    this._salesReboundService.deleteReturnSales(this.params.data.id).subscribe(res => {
