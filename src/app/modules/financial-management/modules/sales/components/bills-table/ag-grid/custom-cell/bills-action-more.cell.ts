...
export class BillsMoreActionsCell extends CoreBaseComponent implements ICellRendererAngularComp {
  params!: ICellRendererParams;
  constructor(@Inject(INJECTOR) injector: Injector, private _router: Router, private _salesService: SalesService) {
    super(injector);
  }
  ...
  updateBill() {
    this._router.navigateByUrl(`${AppRoutes.layout}/${AppRoutes.financial}/sales/update-bill/${this.params.data.id}`);
  }

  deleteBill() {
    this._salesService.deleteSales(this.params.data.id).subscribe(res => {
