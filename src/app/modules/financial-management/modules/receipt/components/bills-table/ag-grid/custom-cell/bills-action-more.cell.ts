...
export class ReceiptMoreActionsCell extends CoreBaseComponent implements ICellRendererAngularComp {
  ...
  updateBill() {
    this._router.navigateByUrl(
      `${AppRoutes.layout}/${AppRoutes.financial}/receipt/update-document/${this.params.data.id}`
    );
  }

  deleteBill() {
    const toastData: IToastData = {
      messageContent: 'you_want_to_delete_receipt',
    ...
    this._toast
      ...
      .pipe(
        untilDestroyed(this),
        switchMap(res => (res ? this._receiptService.deleteReceipt(this.params.data.id) : of(null)))
      )
      .subscribe(res => {
        if (res) {
