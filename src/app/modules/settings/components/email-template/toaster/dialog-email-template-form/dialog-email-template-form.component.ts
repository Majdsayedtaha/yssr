...
@UntilDestroy()
@Component({
  selector: 'dialog-email-template-form',
  templateUrl: './dialog-email-template-form.component.html',
  styleUrls: ['./dialog-email-template-form.component.scss'],
})
export class DialogEmailTemplateFormComponent extends CoreBaseComponent {
  //#region Variables
  ...
  constructor(
    ...
    private _dialog: MatDialogRef<DialogEmailTemplateFormComponent>
  ) {
    super(injector);
  }
  ...
  ngOnInit(): void {
    ...
    if (this.passParamsData) {
      this.editMode = true;
      this.fetchEmailTemplateInfo(this.passParamsData.id);
    }
  }
  //#end region
  ...
  //#region Fetch
  fetchEmailTemplateInfo(Id: string) {
  ...
  submit() {
    ...
    if (this.form.valid) {
      ...
      if (this.editMode) {
        //Edit Mode
        this._templateService
          .editEmailTemplate(this.form.value, this.template.id)
          .pipe(untilDestroyed(this))
          .subscribe({
        ...
        //Add Mode
        this._templateService
          .createEmailTemplate(this.form.value)
          .pipe(untilDestroyed(this))
          .subscribe({next:(response: IResponse<any>) => {
            this.template = response.data;
            this.loading = false;
            this.closeDialog();
          },   error: () => {
