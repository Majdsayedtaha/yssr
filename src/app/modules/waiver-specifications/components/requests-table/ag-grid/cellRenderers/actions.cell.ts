...
export class WaiverMoreActionsCell extends CoreBaseComponent implements ICellRendererAngularComp {
  ...
  openUpdateContract() {
    this._router.navigateByUrl(`${AppRoutes.layout}/${AppRoutes.waiversSpecifications}/update/${this.params.data.id}`);
  }
  ...
  deleteRequestConfirmation() {
    ...
    this._toast
      ...
      .pipe(
        switchMap(res => {
          if (res) {
            return this._waiverSpecificationService.deleteWaiverRequest(this.params.data.id);
          } else {
            return of(null);
          }
        })
